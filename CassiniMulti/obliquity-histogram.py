...
ax[0, 0].hist(init_bobl, bins=bins, color=colors.orange)
ax[0, 0].set_title('LP 890-9 b')
ax[0, 1].hist(init_cobl, bins=bins, color=colors.pale_blue)
ax[0, 1].set_title('LP 890-9 c')
ax[1, 0].hist(final_bobl, bins=5*bins, color=colors.orange)
ax[1, 0].set_xlim(0., 0.25)
ax[1, 1].hist(final_cobl, bins=5*bins, color=colors.pale_blue)
ax[1, 1].set_xlim(0., 6.)
for a in ax[0]:
    a.axhline(number_sims/bins, ls='--', c='k')
    a.set_xlim(0., 180.)
